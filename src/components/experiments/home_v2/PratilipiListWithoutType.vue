...
import PratilipiComponent from '@/components/experiments/home_v2/PratilipiWithoutType.vue'
    ...
    methods: {
        ...
        handleBeforeChange() {
            if (this.$route.meta.store === 'homepage') {
                this.triggerAnanlyticsEvent(`SWIPE_COLLECTIONS_HOME`, 'VD2', {
        ...
        triggerListLink() {
            this.triggerAnanlyticsEvent(`CLICKCOLLECTION_${this.screenLocation}_${this.screenName}`, 'VD2', {
                'USER_ID': this.getUserDetails.userId,
                'PARENT_ID': this.listPageUrl,
        ...
        triggerMoreListLink() {
            this.triggerAnanlyticsEvent(`CLICKMORECOLLECTION_${this.screenLocation}_${this.screenName}`, 'VD2', {
                'USER_ID': this.getUserDetails.userId,
                'PARENT_ID': this.listPageUrl,
                'POSITION': this.position
            });
        ...
        'inViewport.now': function(visible) {
            if (visible) {
                if (this.screenName === 'HOME') {
                    this.triggerAnanlyticsEvent(`VIEWED_COLLECTIONS_HOME`, 'VD2', {
                        'USER_ID': this.getUserDetails.userId,
                        'PARENT_ID': this.listPageUrl,
                        'POSITION': this.position
                    });
