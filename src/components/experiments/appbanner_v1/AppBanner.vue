...
export default {
    ...
    methods: {
        closeBanner() {
            ...
            this.triggerAnanlyticsEvent(`DISMISS_APPBANNER_GLOBAL`, 'WAB001', {
        ...
        downloadApp() {
            this.click_count++;
            const SCREEN_NAME = this.getAnalyticsPageSource(this.$route.meta.store);
            this.triggerAnanlyticsEvent(`GETANDROID_APPBANNER_GLOBAL`, 'WAB001', {
                'USER_ID': this.getUserDetails.userId,
                SCREEN_NAME
            });
    ...
    watch: {
        'inViewport.now': function(visible) {
            if (visible && this.showBanner && this.isAndroid()) {
                const SCREEN_NAME = this.getAnalyticsPageSource(this.$route.meta.store);
                this.triggerAnanlyticsEvent(`VIEWED_APPBANNER_GLOBAL`, 'WAB001', {
                    'USER_ID': this.getUserDetails.userId,
                    SCREEN_NAME
                });
            }
